...
export async function POST(req: NextRequest) {
  try {
    ...
    // For now, we treat "statedBenefit" and "otherCharges" as 0.
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const buyerRequest = await tx.buyerRequest.create({
        data: {
          buyerId: user.id,
          productUrl: productLink,
          paymentLink: productLink, // placeholder until separate payment link is ready
