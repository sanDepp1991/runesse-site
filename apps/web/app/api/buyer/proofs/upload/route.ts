// apps/web/app/api/buyer/proofs/upload/route.ts
import { NextRequest, NextResponse } from "next/server";
import { prisma } from "@runesse/db";
import { supabaseServerClient } from "../../../../../lib/supabaseServer";
import { LedgerScope, LedgerEventType, UserRole } from "@prisma/client";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ✅ NEW STATUS CHECK
    //
    // Your enum is NewRequestStatus:
    //   SUBMITTED | ADMIN_APPROVED | MATCHED | COMPLETED | REJECTED | CANCELLED
    //
    // Buyer uploads should be allowed while the request is still "open":
    //   SUBMITTED, ADMIN_APPROVED, MATCHED
    //
    const status = String(existing.status || "").toUpperCase();
    const openStatuses = ["SUBMITTED", "ADMIN_APPROVED", "MATCHED"];
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Buyer proofs can only be uploaded while the request is SUBMITTED, ADMIN_APPROVED, or MATCHED.",
        },
        { status: 400 }
      );
    }
