...
export default function BuyerBankPage() {
  ...
  React.useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/buyer/bank", { headers: await getAuthHeaders() });
  ...
  async function handleVerify() {
    ...
    try {
      const res = await fetch("/api/buyer/bank/verify", {
          method: "POST",
      ...
      <div className="mx-auto max-w-xl px-4 py-6 space-y-6">
        <section className="rounded-2xl border border-neutral-800 bg-neutral-900/70 p-5">
          ...
          <p className="mt-2 text-xs text-neutral-300">
            Add the bank account where you want refunds for
            returns/cancellations to be credited. In future phases, this will be
            ...
            <div className="mt-3 flex items-center justify-between gap-3 rounded-full border border-neutral-700 bg-black/60 px-3 py-1 text-[11px]">
              ...
              {account.status === "PENDING" && (
                <button
                  ...
                  disabled={verifying}
                  className="rounded-full border border-emerald-500/60 px-3 py-1.5 text-[11px] font-medium text-emerald-200 disabled:opacity-50"
                >
          ...
          {loading ? (
            ...
            <form onSubmit={handleSubmit} className="mt-4 space-y-3">
              ...
              <div className="space-y-1">
                ...
                <input
                  className="w-full rounded-lg border border-neutral-700 bg-black/40 px-3 py-1.5 text-xs outline-none focus:border-emerald-500"
              ...
              <div className="space-y-1">
                ...
                <input
                  className="w-full rounded-lg border border-neutral-700 bg-black/40 px-3 py-1.5 text-xs uppercase outline-none focus:border-emerald-500"
                  value={form.ifsc}
